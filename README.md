...
### Performance
Using an i5-6600 quad-core running at 3.3 GHz, running the `dao/arena2` set of 910 scenarios on a 281x209 grid takes 13 ms using JPS allowing diagonals and with improved pruning disabled. Using default neighbor generation as in normal A* (enabled by setting `GRAPH_PRUNING = false`) makes this take 1.37 s, a factor 100 difference. As a rule, the relative difference increases as maps get larger, with the `dao/arena` set (a smaller map) taking 768 us and 1.34 ms respectively with and without pruning. Without diagonals, running `dao/arena2` takes 89 ms with pruning and 727 ms without. 
